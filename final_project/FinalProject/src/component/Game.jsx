...
const game = (props) => {
  ...
  const saveGame = () => {
    setEditMode(false);
    const updatedGame = {GameName:GameName, ConsoleName:ConsoleName, Developer:Developer, Score:Score, id:props.game.id, image:props.game.image};
  ...
  return (
    ...
    {!editMode &&     <ul className='list-group list-group-flush'>
      ...
      <li className='list-group-item text-center'>{props.game.Score}</li>
...
{editMode &&     <ul className='list-group list-group-flush'>
      ...
      <li className='list-group-item text-center'><input type='text' className='form-control' value={Score} onChange={(e) => setScore(e.currentTarget.value)} /></li>
